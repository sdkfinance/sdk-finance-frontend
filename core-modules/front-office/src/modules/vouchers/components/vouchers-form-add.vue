  ...
  <app-form
    ...
    <template v-else>
      <div class="form__list">
        ...
        <p>
          <strong>{{ $t('pages.vouchers.voucher_number') }}: </strong>{{ voucherData.number }}
        </p>
        <p>
          <strong>{{ $t('pages.vouchers.pin') }}: </strong>{{ voucherData.pin }}
        </p>
        <p>
          <strong>{{ $t('pages.vouchers.voucher_amount') }}: </strong>{{ voucherData.amount }} {{ commissions.currency }}
        </p>
      </div>
    </template>
  </app-form>
...
<script lang="ts">
import type { IVoucherCreated, IVoucherNew, IWalletRecord } from '@sdk5/shared/requests';
import { VouchersRequests, WalletsRequests } from '@sdk5/shared/requests';
import type { IPlainObject } from '@sdk5/shared/types';
import { errorNotification, successNotification } from '@sdk5/shared/utils';
import { SimpleNumberValidationRule, SimpleRequiredValidationRule } from '@sdk5/shared/validation';
...
export default class VouchersFormAdd extends Vue {
  ...
  protected async calculateCommission(): Promise<void> {
    ...
    this.commissions.currency = response?.currency.code;
  }
  ...
  protected async handleForm(): Promise<void> {
    ...
    this.isLoading = true;
    const { response, error } = await VouchersRequests.createVoucher(this.form);
    this.isLoading = false;
