...
import { AppButton, AppButtonsGroup, AppForm, AppFormItem, AppInput, AppSelect, AppSwitch } from '@sdk5/ui-kit-front-office';
...
import CommissionBlock from './commission-block.vue';
import WithdrawalSuccess from './withdrawal-success.vue';
...
export default class WithdrawalForm extends Vue {
  ...
  protected get allowedWithdrawalMethods() {
    ...
    return isCashDeskHidden ? withdrawalMethodsList.filter(({ value }) => value !== 'cash_desk') : withdrawalMethodsList;
  }
  ...
  protected async onMainChange(needFetchingData: boolean = false, resetFields: Array<TWithdrawalFormResetFields> = []): Promise<void> {
    await this.$nextTick();
    ...
    if (resetFields.length) {
      resetFields.forEach((key: TWithdrawalFormResetFields) => {
        this.form[key] = null;
      });
    }
  ...
  protected async withdrawalProviderSubmit(): Promise<void> {
    ...
    const {
      gateProvider: { id: gateProviderId },
      way,
    } = accountMethod;
    const sendData = {
      amount,
  ...
  protected async withdrawalBankSubmit(): Promise<void> {
    const { amount, coin, bankAccountNumber, address, bic, fullName, iban, name, swift } = this.form;

    if (!amount || !coin || !fullName) {
      return;
    }
  ...
  protected async withdrawalCashDeskSubmit(): Promise<void> {
    const { coin, amount, receiverName, cashDeskId } = this.form;

    if (!coin || !amount || !receiverName || !cashDeskId) {
      return;
    }
    ...
    const { cashAmount, clientCoin, requestIdentifier } = response.process;

    const {
      currency: { name: currency },
      serial,
    } = clientCoin as IClientCoin;
  ...
  protected async withdrawalProviderCalculate(): Promise<void> {
    const { accountMethod, coin: serial, amount, txType } = this.form;

    if (!accountMethod || !serial || !amount || !txType) {
      return;
    }

    const { response, error } = await GateRequests.calculateCommission({
      gateProviderId: accountMethod.gateProvider.id,
      serial,
      amount,
      txType,
    });
  ...
  protected async withdrawalBankCalculate(): Promise<void> {
    ...
    const {
      commissionAmountPush: commissionAmount,
      recipientAmountPush: amountToSend,
      currency: { code: currency },
    } = response;
    this.commission = { amountToSend, commissionAmount, currency };
  }
