...
<Meta title="ui-framework/AppUpload" component={ AppUpload }/>
...
### Default
<Canvas>
  <Story name="Default view" args={{ action: config.action, tip: config.tip, label: config.label }}>
    { Template.bind({}) }
  </Story>
</Canvas>
...
### Loading
<Canvas>
  <Story name="Loading view" args={{ action: config.action, tip: config.tip, label: config.label, isLoading: true }}>
    { Template.bind({}) }
  </Story>
</Canvas>
