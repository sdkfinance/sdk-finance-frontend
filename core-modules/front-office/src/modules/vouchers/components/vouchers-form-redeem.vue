...
<script lang="ts">
import type { IPostVoucherRedeemForm, IVoucherRedeem, IWalletRecord } from '@sdk5/shared/requests';
...
export default class VouchersFormRedeem extends Vue {
  ...
  protected async fetchViewPrepaidVoucher(): Promise<void> {
    ...
    this.prepaidVoucherId = response?.currency?.id;
  ...
  protected get destSerial(): string | null {
    return this.coinsData.find((coin) => coin.currency.id === this.prepaidVoucherId)?.serial || null;
  }
  ...
  protected async calculateCommission(): Promise<void> {
    ...
    const { prepaidPin, serial } = this.form;
