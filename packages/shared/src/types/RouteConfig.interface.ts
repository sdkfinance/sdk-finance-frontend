...
import type { Route, RouteConfigMultipleViews, RouteConfigSingleView } from 'vue-router/types/router';

import type { TRole } from '../constants/roles';
import type { TAppBreadcrumb } from './app-breadcrumbs.types';
...
export type TSetBreadcrumbsFn<T = Record<string, any>> = (route: Route, options?: T) => TAppBreadcrumb[];
...
export interface IMetaConfig {
  ...
  */;
  userProfilePermission?: TRole[] | TPermissionFn /**
   ...
   function = custom checker return boolean type for this user role
   */;
  translation?: string; // label text for dashboard menu
